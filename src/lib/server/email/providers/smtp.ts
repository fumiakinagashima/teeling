...
export async function sendSmtp(config: SmtpConfig, mail: Mail): Promise<void> {
	...
	try {
		...
		throw new Error(
			'SMTP sending is only available in the Cloudflare Workers runtime (not available in the local `bun run dev` environment). Please try it in production (after deploying) or with `wrangler dev`. For local testing, use Resend or AWS SES instead.'
		);
	}
