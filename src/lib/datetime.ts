 ...
 * into the corresponding Date (UTC instant).
 * Explicitly applies a +09:00 offset instead of using `new Date(value)`, so the result does not
 * depend on the runtime's local timezone (Cloudflare Workers always run in UTC).
 */
